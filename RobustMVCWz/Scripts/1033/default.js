...
function OnFinish(selProj, selObj) {
    try {
        ...
        //var strProjTemplatePathRoot = wizard.FindSymbol('PROJECT_TEMPLATE_PATH');
        var strProjTemplatePathRoot = wizard.FindSymbol('START_PATH');

        selProj0 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\default.vcxproj', 'default', strSolutionPath + '\\default');
        selProj0.Object.Save();

        var oFSO = new ActiveXObject("Scripting.FileSystemObject");

        oFSO.CopyFolder(strProjTemplatePathRoot + '\\Templates\\1033\\props', strProjectPath + '\\..\\props');

        selProj1 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\Templates\\1033\\Core\\Core.vcxproj', 'Core', strSolutionPath + '\\Core');
        selProj1.Object.Save();

        selProj2 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\Templates\\1033\\Core.Test\\Core.Test.vcxproj', 'Core.Test', strSolutionPath + '\\Core.Test');
        selProj2.Object.References.Item(5).Remove();
        selProj2.Object.References.AddProject(selProj1);
        selProj2.Object.Save();

        selProj4 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\Templates\\1033\\Visualization\\Visualization.vcxproj', 'Visualization', strSolutionPath + '\\Visualization');
        selProj4.Object.Save();

        selProj3 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\Templates\\1033\\Application\\Application.vcxproj', 'Application', strSolutionPath + '\\Application');
        selProj3.Object.References.Item(2).Remove();
        selProj3.Object.References.Item(1).Remove();
        selProj3.Object.References.AddProject(selProj1);
        selProj3.Object.References.AddProject(selProj4);
        selProj3.Object.Save();


        selProj5 = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\Templates\\1033\\Prepare\\Prepare.vcxproj', 'Prepare', strSolutionPath + '\\Prepare');
        selProj5.Object.Save();

        oTarget.Remove(selProj0);


        // nice try -- but does not work
        var StartupProjects = new Array(selProj3.UniqueName);
        oTarget.SolutionBuild.StartupProjects = StartupProjects;

        oFSO.CopyFolder(strProjTemplatePathRoot + '\\Templates\\1033\\ext', strProjectPath + '\\..\\ext');


        //		selProj = CreateCustomProjectXXX(strProjTemplatePathRoot + '\\default.vcxproj', strProjectName, strProjectPath);
...
function CreateCustomProjectXXX(strProjectTemplatePath, strProjectName, strProjectPath) {
    try {
        var Solution = dte.Solution;
        var strSolutionName = "";
        ...
        {
            var prjItem = oTarget.AddFromTemplate(strProjectTemplatePath, strProjectNameWithExt);
            prj = prjItem.SubProject;
        }
        else {
            prj = oTarget.AddFromTemplate(strProjectTemplatePath, strProjectPath, strProjectNameWithExt);
        }
